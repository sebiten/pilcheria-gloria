-- Valida el resumen confirmado sin usarlo como fuente autoritativa de importes.
create or replace function public.create_checkout_order(
  ...
  p_expected_shipping_cost numeric,
...
declare
  ...
  legacy_canonical_payload jsonb;
  payload_hash text;
...
begin
  ...
  if p_expected_subtotal is null or p_expected_subtotal < 0
     or p_expected_discount is null or p_expected_discount < 0
     or p_expected_shipping_cost is null or p_expected_shipping_cost < 0
     or p_expected_total is null or p_expected_total < 0 then
    raise exception 'El resumen economico esperado no es valido';
  end if;
  ...
  from (
    select parsed.product_id, parsed.variant_id,
  ...
  if safe_shipping_method = 'local_delivery'
     and (nullif(btrim(p_shipping_address->>'street'), '') is null
       or nullif(btrim(p_shipping_address->>'city'), '') is null) then
    raise exception 'Completa la direccion y localidad para la entrega';
  end if;

  legacy_canonical_payload := jsonb_build_object(
  ...
  legacy_payload_hash := encode(
    extensions.digest(convert_to(legacy_canonical_payload::text, 'UTF8'), 'sha256'),
    'hex'
  );
  canonical_payload := legacy_canonical_payload || jsonb_build_object(
    'expected_discount', round(p_expected_discount, 2),
    'expected_shipping_cost', round(p_expected_shipping_cost, 2),
  ...
  if found then
    if p_clerk_user_id is not null then
      if existing_order.clerk_user_id is distinct from p_clerk_user_id then
       ...
       or coalesce(existing_order.checkout_owner_fingerprint,
    ...
    if existing_order.checkout_payload_hash is null then
      if existing_order.shipping_method is distinct from safe_shipping_method
         ...
         or abs((existing_order.total - coalesce(existing_order.shipping_cost, 0)
             + coalesce(existing_order.discount_total, 0))
           - round(p_expected_subtotal, 2)) > 0.01
         or abs(coalesce(existing_order.discount_total, 0)
           - round(p_expected_discount, 2)) > 0.01
         or abs(coalesce(existing_order.shipping_cost, 0)
           - round(p_expected_shipping_cost, 2)) > 0.01
         or abs(existing_order.total - round(p_expected_total, 2)) > 0.01
      ...
      if existing_order.checkout_payload_hash <> legacy_payload_hash
         or abs(coalesce(existing_order.discount_total, 0)
           - round(p_expected_discount, 2)) > 0.01
         or abs(coalesce(existing_order.shipping_cost, 0)
           - round(p_expected_shipping_cost, 2)) > 0.01
         or abs(existing_order.total - round(p_expected_total, 2)) > 0.01 then
    ...
    if p_clerk_user_id is null then
      ...
      set guest_access_token = null,
          guest_access_token_hash = encode(
            extensions.digest(btrim(p_guest_access_token), 'sha256'), 'hex'
          )
      where id = p_checkout_id
      returning * into existing_order;
    end if;
  ...
  for requested_item in
    select * from jsonb_to_recordset(normalized_items) as item(
      product_id uuid, variant_id uuid, quantity integer
  ...
  loop
    ...
    for offer_record in
      ...
      where offer.variant_id = requested_item.variant_id
        and offer.active and source.active
    ...
    set stock = coalesce((
      ...
      where offer.variant_id = variant.id
        and offer.active and offer.availability_mode = 'finite'
    ), 0)
  ...
  if abs(discount_total - round(p_expected_discount, 2)) > 0.01
     or abs(shipping_cost - round(p_expected_shipping_cost, 2)) > 0.01
     or abs(order_total - round(p_expected_total, 2)) > 0.01 then
    raise exception 'El resumen de la compra cambio. Revisalo antes de pagar';
  end if;
  ...
  insert into public.orders (
    id, clerk_user_id, total, shipping_cost, shipping_method, shipping_address,
    guest_access_token, guest_access_token_hash, coupon_code, discount_total,
    status, reservation_expires_at, stock_reserved, stock_restored,
    coupon_counted, analytics_session_id, checkout_payload_hash,
    checkout_owner_fingerprint
  ) values (
    p_checkout_id, p_clerk_user_id, order_total, shipping_cost,
    safe_shipping_method, checkout_address, null,
    case when p_clerk_user_id is null then encode(
      extensions.digest(btrim(p_guest_access_token), 'sha256'), 'hex'
    ) else null end,
    normalized_coupon, discount_total, 'pending', now() + interval '30 minutes',
    true, false, coupon_counted, p_analytics_session_id, payload_hash,
  ...
  loop
    ...
    allocated_discount_cents := case
      ...
      else least(remaining_discount_cents, case
    ...
    insert into public.order_items (
      order_id, product_id, variant_id, offer_id, source_id, source_code,
      source_name, availability_mode, seller_share_rate, quantity, unit_price,
      line_subtotal, discount_allocated, net_amount, seller_share,
      partner_share, procurement_status
    ) values (
      p_checkout_id,
      ...
      resolved_item->>'source_code', resolved_item->>'source_name',
      ...
      case when resolved_item->>'availability_mode' = 'on_demand'
        then 'awaiting_payment' else 'not_required' end
    );
  end loop;
...
revoke all on function public.create_checkout_order(
  ...
  numeric, numeric, numeric, numeric, uuid, boolean, boolean
...
comment on function public.create_checkout_order(
  ...
  'Crea o recupera un checkout idempotente, valida el resumen economico completo y reserva stock/cupon en una unica transaccion.';

drop function public.create_checkout_order(
  uuid, text, text, text, jsonb, text, jsonb, text,
  numeric, uuid, boolean, boolean
);
