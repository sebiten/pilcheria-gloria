...
alter table public.storefront_analytics_events
  ...
  add constraint storefront_analytics_events_event_detail_check check (
    (
      event_name = 'checkout_blocked'
      and analytics_version >= 7
      and event_detail in (
        'cart_refresh_failed',
        'item_unavailable',
        'no_shipping_method',
    ...
    or (
      ...
      and (
        ...
        or (analytics_version < 7 and event_detail is null)
      )
    )
    or (
      ...
      and (
        ...
        or (
          char_length(event_detail) between 1 and 100
...
session_flags as (
  select
    ...
    bool_or(event.event_name = 'checkout_ready') as checkout_ready,
    bool_or(event.event_name = 'checkout_blocked') as checkout_blocked,
    bool_or(event.event_name = 'checkout_form_started') as checkout_form_started,
...
checkout_blocker_stats as (
  select event.event_detail as detail, count(distinct event.session_id) as sessions
  from events as event
  where event.event_name = 'checkout_blocked' and event.event_detail is not null
  group by event.event_detail
  order by sessions desc, event.event_detail asc
),
...
select jsonb_build_object(
  ...
  'comparable_started_at', (
    select min(created_at) from public.storefront_analytics_events where analytics_version >= 7
  ),
  ...
  'metrics', jsonb_build_object(
    ...
    'checkout_ready_sessions', coalesce((select count(*) from session_flags where checkout_ready), 0),
    'checkout_blocked_sessions', coalesce((select count(*) from session_flags where checkout_blocked), 0),
    'checkout_form_started_sessions', coalesce((select count(*) from session_flags where checkout_form_started), 0),
  ...
  'checkout_blockers', coalesce((select jsonb_agg(to_jsonb(checkout_blocker_stats) order by sessions desc) from checkout_blocker_stats), '[]'::jsonb),
...
events as (
  ...
  from public.storefront_analytics_events as event
  cross join params
  where event.created_at >= params.start_at and event.analytics_version >= 7
),
session_flags as (
  select
    event.session_id,
